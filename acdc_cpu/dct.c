...
bool Tensor_run_dct(PyArrayObject* input0, PyArrayObject** output0)
{
    ...
    for (int i = 0; i < batch_size; ++i) {
        DTYPE_OUTPUT_0* output_data_ptr = (DTYPE_OUTPUT_0*)PyArray_DATA(*output0);
        DTYPE_OUTPUT_0* example = output_data_ptr + i * input_size;
...
int APPLY_SPECIFIC(DCT_updateOutput)(PyArrayObject* input0,
                                     PyArrayObject** output0) {

    if (!Tensor_run_dct(input0, output0)) {
        PyErr_Format(PyExc_ValueError,
                    "DCT updateOutput failed");
        return 1;
    }
    return 0;
}
